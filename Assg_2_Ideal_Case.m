...
% THIS IS FOR THE IDEAL GAS CASE!!! The non-ideal case is present in
% Q.1)a,b,c files.
...
% The following are the Antoine coeff parameters for Propane only!!
%global B_prop
B_prop= 1441.629;
%global C_prop
C_prop= -74.299;
...
T = 350;
global p_sat_prop
p_sat_prop = 10^(A_prop - (B_prop/(T + C_prop)));
...
C_water = -198.043;
global p_sat_water
p_sat_water = 10^(A_water - (B_water/(T + C_water)));

global k
k = p_sat_prop/p_sat_water;


%global P_sat
%P_sat = 10^(A - (B./(T + C))); % Antoine's eqn!!
...
% This is extra, we needed an extra input for that
...
while(x_new >= x_bottom)
    solve_for_hori_inter = @(x) y_val - k*x/(1+ (k-1)*x);
...
function y = calc_y_from_x(x)  %subs = 'p' or 'w'. P = Total pressure
...
%         y = P_sat*x/P;
%     elseif(subs == 'w')
%         error("This is not supported right now. Needs to be developed :P");
%         %y = gamma2*x/P;
%     else
